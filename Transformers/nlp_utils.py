...
def ner_iob2_disc_annotate(arr_start_end, df_ann, subtask='ner'):
    ...
    for index, row in df_ann.iterrows():
        ...
        if subtask == 'ner': iob_disc_labels[tok_start] = "B"
        elif subtask == 'norm': iob_disc_labels[tok_start] = "B" + "-" + row['code']
        else: raise Exception('Subtask not implemented!')
        ...
        if tok_start < tok_end:
            ...
            for i in range(tok_start + 1, tok_end + 1):
                ...
                if subtask == 'ner': iob_disc_labels[i] = "I"
                elif subtask == 'norm': iob_disc_labels[i] = "I" + "-" + row['code']
        ...
        for ann_i in range(1, ann_loc_len):
            ...
            # Annotate first subtoken/word
            assert iob_disc_labels[tok_start] == "O" # no overlapping annotations are expected
            if subtask == 'ner': iob_disc_labels[tok_start] = "I"
            elif subtask == 'norm': iob_disc_labels[tok_start] = "I" + "-" + row['code']
...
def start_end_tokenize_ner(text, max_seq_len, tokenizer, start_pos, df_ann, ign_value, strategy="word-all", cased=True, word_pos=0, 
    ...
    if strategy.split('-')[0] == "word":
        ...
        if len(subtask.split('-')) == 1:
            ...
            word_labels = ner_iob2_disc_annotate(arr_start_end=np.array(word_chr_start_end), df_ann=df_ann, subtask=subtask.split('-')[0])
...
def ss_sep_start_end_tokenize_ner(ss_start_end, max_seq_len, text, tokenizer, df_ann, ign_value, strategy="word-all", 
    ...
    txt_sub_token, txt_start_end, txt_labels, txt_word_id = start_end_tokenize_ner(text=text, max_seq_len=int(1e22),
                            tokenizer=tokenizer, start_pos=0, df_ann=df_ann, ign_value=ign_value, 
...
def ss_create_input_data_ner(df_text, text_col, df_ann, df_ann_text, doc_list, ss_dict, tokenizer, lab_encoder_list, text_label_encoder, seq_len, ign_value, 
    ...
    for doc in doc_list:
        ...
        if ss_dict is not None:
            ...
            if ss_sep:
                doc_ss_token, doc_ss_start_end, doc_ss_label, doc_ss_word_id = ss_sep_start_end_tokenize_ner(ss_start_end=doc_ss, 
...
def extract_seq_preds(arr_doc_seq_preds, arr_doc_seq_start_end, doc_list, lab_decoder_list, df_text, text_col, 
    ...
    for d in range(len(doc_list)):
        ...
        doc_seq_lab = []
        if len(subtask.split('-')) == 1 or subtask.split('-')[1] in ('mention', 'iob_disc'): # ner, norm, norm-mention, ner-iob_disc, norm-iob_disc
            # Single label (custom)
            ...
            if len(subtask.split('-')) == 1 or subtask.split('-')[1] != "iob_disc":
                ...
                ann_res.extend(ner_iob2_disc_extract_seq_preds(doc_id=doc, seq_preds=doc_seq_lab, seq_start_end=doc_seq_start_end, 
                                                          df_text=df_text, text_col=text_col, subtask=subtask.split('-')[0], strategy=strategy))
